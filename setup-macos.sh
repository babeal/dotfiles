...
_mainScript_() {
  ...
  _symlinks_() {
    ...
    _makeSymlink_ "${gitRoot}/bash_profile"         "${HOME}/.bash_profile"
    _makeSymlink_ "${gitRoot}/bashrc"               "${HOME}/.bashrc"
    ...
    _makeSymlink_ "${gitRoot}/zshrc"                "${HOME}/.zshrc"
  ...
  _homebrew_() {
    ...
    # https://dev.to/code2bits/homebrew---basics--cheatsheet-3a3n
    ...
    _execute_ -v "caffeinate -ism brew update" # https://ss64.com/osx/caffeinate.html
    ...
    #Base Install
      ...
      _execute_ -vp "brew install autoconf" # https://www.gnu.org/software/autoconf/
      _execute_ -vp "brew install autojump" # https://github.com/wting/autojump - faster way to navigate your filesystem
      _execute_ -vp "brew install automake" # https://www.gnu.org/software/automake/ 
      # _execute_ -vp "brew install bash-completion@2" # https://github.com/scop/bash-completion
      _execute_ -vp "brew install bash" # upgrades bash to the latest version.  Mac version is 3.2 which is from 2007
      _execute_ -vp "brew install bashdb" # https://formulae.brew.sh/formula/bashdb - bash debugger
      _execute_ -vp "brew install bat" # https://github.com/sharkdp/bat - better version of cat
      _execute_ -vp "brew install bats" # https://github.com/sstephenson/bats - bash automated testing system
      _execute_ -vp "brew install colordiff" # https://www.colordiff.org/ - diff but with colors
      _execute_ -vp "brew install coreutils" # https://www.gnu.org/software/coreutils - gnu core utilities
      _execute_ -vp "brew install curl" # https://curl.haxx.se/ - updates curl
      _execute_ -vp "brew install exa" # https://github.com/ogham/exa - modern replacement for ls
      _execute_ -vp "brew install fping" # https://fping.org/ - more powerful ping program
      _execute_ -vp "brew install git-extras" # https://github.com/tj/git-extras - git helper commands
      # _execute_ -vp "brew install git-flow"
      _execute_ -vp "brew install git" # installs the latest version of git
      _execute_ -vp "brew install gnu-sed" # https://www.gnu.org/software/sed/
      _execute_ -vp "brew install gpg" # needed by asdf-nodejs
      # _execute_ -vp "brew install gnupg" # https://gnupg.org/ - GNU privacy guard (encrypting messages)
      _execute_ -vp "brew install htop" # https://htop.dev/ - interactive process viewer
      _execute_ -vp "brew install httpie" # https://httpie.io/ - friendly curl replacement
      _execute_ -vp "brew install jq" # https://stedolan.github.io/jq/ - lightweight and flexible command-line JSON parser
      _execute_ -vp "brew install lesspipe" # https://www-zeuthen.desy.de/~friebel/unix/lesspipe.html - input filter for less 
      _execute_ -vp "brew install libtool" # https://www.gnu.org/software/libtool/
      _execute_ -vp "brew install mas" # https://github.com/mas-cli/mas - command line interface for the mac app store
      # _execute_ -vp "brew install micro" # https://github.com/zyedidia/micro - terminal based text editor
      _execute_ -vp "brew install openssl" # https://www.openssl.org/ - cryptography library
      _execute_ -vp "brew install prettyping" # https://denilson.sa.nom.br/prettyping/ - wrapper around the ping tool
      _execute_ -vp "brew install readline" # https://tiswww.case.edu/php/chet/readline/rltop.html - gnu readline
      _execute_ -vp "brew install shellcheck" # https://www.shellcheck.net/ - finds bugs in your shell scripts
      _execute_ -vp "brew install shfmt" # https://github.com/mvdan/sh - shell parser, formatter and interpreter
      _execute_ -vp "brew install source-highlight" # https://www.gnu.org/software/src-highlite/ - gnu source highlight
      _execute_ -vp "brew install ssh-copy-id" # https://formulae.brew.sh/formula/ssh-copy-id - unknown
      # _execute_ -vp "brew install thefuck" # https://github.com/nvbn/thefuck - corrects errors in previous console commands
      _execute_ -vp "brew install tldr" # https://github.com/tldr-pages/tldr - collaborative cheat sheets
...
_checkASDF_() {
  ...
  if [ ! -d "${HOME}/.asdf" ]; then
    ...
    local asdfPath=$(brew --prefix asdf)
    [[ -s "${asdfPath}/asdf.sh" ]] \
      && source "${asdfPath}/asdf.sh" \
      || {
    ...
    # shellcheck disable=SC2015
    [[ -s "${asdfPath}/etc/completions/asdf.bash" ]] \
      && source "${asdfPath}/etc/completions/asdf.bash" \
      || {
        error "Could not source '${asdfPath}/completions/asdf.bash'" "${LINENO}"
        return 1
      }
  fi
}
...
_parseOptions_() {
  ...
  while (($#)); do
    case $1 in
      ...
      # TODO: this isn't working as an option must be supplied for the script to run
      *) options+=("$1") ;;
    esac
    shift
  done
